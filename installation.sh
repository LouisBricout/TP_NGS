...
#This code is preliminary to any data processing.
#It downloads executables which will be subsequently necessary and makes them easy to run from any machine.
...
# Export the path of the executable (such that fastqc can be launched from anywhere)
cd FastQC
chmod 755 fastqc
...
# Export the path of the executable (such that samtools can be launched from anywhere)
...
# Export the path of the executable (such that igv.sh can be launched from anywhere)
cd IGV_2.4.14
mv igv.sh igv
chmod 755 igv
echo 'export PATH='$(pwd)':${PATH}' >> ~/.bashrc
